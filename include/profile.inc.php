<?php   
  ...
  $query2 = "SELECT * FROM `posts` JOIN user ON user.id_user=posts.id_user AND  posts.id_user IN 
  (SELECT user.id_user  FROM followers JOIN user ON user.id_user=followers.id_follower  AND followers.id_user=$id_user )
  UNION
  SELECT * FROM posts JOIN user ON user.id_user=posts.id_user AND posts.id_user= $id_user
...
if(isset($_GET['id_profile'])){
  ...
  $query3 = "SELECT `nom`, `prenom`, `photo`, `description` FROM `user` WHERE id_user = ".$id_profile;
  ...
  if(isset($_POST['act'])){
      ...
      $ressql = mysqli_query($link,$query) or die(mysqli_query($link));
